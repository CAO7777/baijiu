// static/script.js (v4.3 - Final, Fully Integrated Version)

document.addEventListener('DOMContentLoaded', () => {
    // --- 1. STATE MANAGEMENT ---
    let personas = [];
    let currentPersona = null; // Used for editing mode
    let individualReportsRendered = 0;
    let productImageDataURL = null;

    // --- 2. DOM ELEMENT SELECTORS (Complete & Corrected for 4-step layout) ---
    ...
    // Step 3 (Individual Reports Page) Elements
    ...
    const loadPersonaForEdit = (index) => {
        ...
        genderEl.value = currentPersona.gender;
        ageEl.value = currentPersona.age;
        cityEl.value = currentPersona.city;
        professionEl.value = currentPersona.profession;
        eduEl.value = currentPersona.education || '';
        incomeEl.value = currentPersona.income || '';
        priceEl.value = currentPersona.expected_price || '';
        drinkFreqEl.value = currentPersona.drink_frequency || '';
        drinkYearsEl.value = currentPersona.drinking_history || '';
        flavorEl.value = currentPersona.preferred_aroma || '';
        ...
        addPersonaBtn.classList.remove('outline');
        addPersonaBtn.classList.add('primary');

        let cancelBtn = document.getElementById('cancelEditPersona');
        if (!cancelBtn) {
            cancelBtn = document.createElement('button');
            cancelBtn.id = 'cancelEditPersona';
    ...
    const cancelEditPersona = () => {
        ...
        addPersonaBtn.classList.add('outline');
        addPersonaBtn.classList.remove('primary');
    ...
    const addOrUpdatePersona = () => {
        if (currentPersona) {
            ...
            if (indexToUpdate !== -1) {
                personas[indexToUpdate] = updatedPersona;
            }
            cancelEditPersona();
        } else {
            const newPersona = collectPersonaPayload();
    ...
    const generateReport = async () => {
        ...
        const nextReportLoaderHTML = `<div id="next-report-loader" class="persona-report-loading"><div class="spinner-small"></div><span>正在生成下一个画像分析报告...</span></div>`;
                ...
                switch (message.type) {
                    case 'individual_report':
                        ...
                        if (individualReportsRendered === personas.length) {
                            ...
                            individualContainer.insertAdjacentHTML('beforeend', nextReportLoaderHTML);
                        }
                        break;
                    ...
                    case 'done': 
                        eventSource.close();
                        const finalLoader = document.getElementById('next-report-loader');
        ...
        reportEl.innerHTML = `
            ...
            <div class="report-details-grid">
                <div class="report-text-sections">
                    ...
                    <div class="report-section">
                        <h5>潜在消费场景</h5>
                        <p>${scenarioAnalysis}</p>
                    </div>
                </div>
                ${data.radar_chart ? `
                <div class="radar-chart-container">
                    <h5>画像-产品匹配度雷达图</h5> <img src="data:image/png;base64,${data.radar_chart}" alt="画像-产品匹配度雷达图">
                </div>
                ` : ''}
            </div>
