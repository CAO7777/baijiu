...
# 1. 检查是否在 Render 生产环境
render_disk_path = os.getenv("RENDER_DISK_MOUNT_PATH")

if render_disk_path:
    BASE_STORAGE_PATH = Path(render_disk_path)
    print(f"检测到 Render 环境，使用永久硬盘路径: {BASE_STORAGE_PATH}")
else:
    BASE_STORAGE_PATH = Path(".")
    print("未检测到 Render 环境，使用本地路径。")

# 2. 在基础路径下创建你的文件夹
ARCHIVE_DIR = BASE_STORAGE_PATH / "archived_reports"
VECTOR_DB_DIR = BASE_STORAGE_PATH / "vector_db"
...
# 3. 💡 新增：归档保留策略 (默认6个月)
ARCHIVE_RETENTION_MONTHS = int(os.getenv("ARCHIVE_RETENTION_MONTHS", 6))
beijing_tz = timezone(timedelta(hours=8)) # 定义北京时间
# --- 结束新增 ---
...
# --- Vector Search (无变动) ---
VECTOR_DB_PATH = VECTOR_DB_DIR / "tfidf_database_real.pkl"
...
def admin_dashboard():
    ...
    archives = []
    # 💡 获取当前时间和N个月前的截止时间
    now_in_beijing = datetime.now(timezone.utc).astimezone(beijing_tz)
    retention_limit = now_in_beijing - relativedelta(months=ARCHIVE_RETENTION_MONTHS)
    ...
    if ARCHIVE_DIR.exists():
        ...
        for report_id in sorted_dirs:
            ...
            report_dt = None
            try:
                # 💡 假设文件夹名称(report_id)已经是北京时间
                report_dt = datetime.strptime(report_id, "%Y%m%d_%H%M%S")
                report_dt = beijing_tz.localize(report_dt) # 告知 datetime 对象它代表的是北京时间
                display_time = report_dt.strftime("%Y-%m-%d %H:%M:%S")
            except ValueError:
                pass # 解析失败，使用原始 ID

            # 💡 1. 自动删除过期报告
            if report_dt and report_dt < retention_limit:
                try:
                    shutil.rmtree(report_dir)
                    print(f"自动删除: 报告 {report_id} (时间 {display_time}) 已超过 {ARCHIVE_RETENTION_MONTHS} 个月。")
                    continue # 删除后，跳过，不添加到列表
                except Exception as e:
                    print(f"自动删除报告 {report_id} 失败: {e}")

            # 💡 2. 读取数字人数量
            persona_count = 0
            personas_file_path = report_dir / "personas.json"
            try:
                if personas_file_path.exists():
                    with open(personas_file_path, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        if isinstance(data, list):
                            persona_count = len(data)
            except Exception as e:
                print(f"读取 persona count 失败 {report_id}: {e}")
            ...
            try:
                ...
                if summary_file not in files:
                    summary_file = "summary_report.html"
                
                archives.append({
                    "id": report_id,
                    "display_time": display_time,
                    "persona_count": persona_count, # 💡 传递数量
    ...
    return render_template('admin_dashboard.html', archives=archives, retention_months=ARCHIVE_RETENTION_MONTHS)
...
# 💡 新增：逐个删除路由
@app.route('/admin_delete_report/<path:report_id>', methods=['POST'])
def admin_delete_report(report_id):
    ...
    try:
        ...
        if '..' in report_id or '/' in report_id or '\\' in report_id:
             print(f"删除失败: 无效的 report_id {report_id}")
             return redirect(url_for('admin_dashboard'))

        report_dir = ARCHIVE_DIR / report_id
        if report_dir.exists() and report_dir.is_dir():
            shutil.rmtree(report_dir)
            print(f"管理员手动删除: {report_id}")
        else:
            print(f"删除失败: 找不到 {report_id}")
    except Exception as e:
        print(f"删除报告 {report_id} 失败: {e}")
    
    return redirect(url_for('admin_dashboard'))


# 💡 新增：按条件批量删除路由
@app.route('/admin_batch_delete', methods=['POST'])
def admin_batch_delete():
    if not session.get('admin_logged_in'):
        return redirect(url_for('admin_login'))
    
    try:
        min_count = int(request.form.get('min_persona_count', 0))
        print(f"管理员批量删除: 数字人数量 < {min_count}")
    except ValueError:
        return redirect(url_for('admin_dashboard')) # 恶意输入
    
    deleted_count = 0
    if ARCHIVE_DIR.exists():
        # 注意：这里我们必须重新遍历所有文件夹
        for report_id in os.listdir(ARCHIVE_DIR):
            report_dir = ARCHIVE_DIR / report_id
            ...
            try:
                ...
                # 检查是否符合删除条件
                if persona_count < min_count:
                    shutil.rmtree(report_dir)
                    print(f"批量删除: {report_id} (count={persona_count})")
                    deleted_count += 1

            except Exception as e:
                print(f"批量删除时出错 {report_id}: {e}")
    
    print(f"批量删除完成，共删除 {deleted_count} 个报告。")
    return redirect(url_for('admin_dashboard'))


# (无变动) 清空所有归档的路由
@app.route('/admin_clear_all', methods=['POST'])
def admin_clear_all():
    if not session.get('admin_logged_in'):
        return redirect(url_for('admin_login'))
    
    print(f"管理员请求清空归档目录: {ARCHIVE_DIR}")
    try:
        ...
        print(f"错误：清空归档目录失败: {e}")
        ARCHIVE_DIR.mkdir(exist_ok=True)
        
    return redirect(url_for('admin_dashboard'))


# --- Final startup logic (无变动) ---
...
if __name__ == '__main__':
    ...
    print(f"归档保留策略: {ARCHIVE_RETENTION_MONTHS} 个月")
