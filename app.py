...
if Path("/var/data").exists():
    STORAGE_ROOT = Path("/var/data")
    print("检测到 Render 挂载盘，使用持久化存储路径: /var/data")
else:
    STORAGE_ROOT = Path(".")
    print("未检测到挂载盘，使用本地临时存储路径")

# 1. 修改归档目录路径
ARCHIVE_DIR = STORAGE_ROOT / "archived_reports"
ARCHIVE_DIR.mkdir(parents=True, exist_ok=True) # 使用 parents=True 以防父目录不存在
...
# --- Vector Search (无变动) ---
VECTOR_DB_PATH = STORAGE_ROOT / "vector_db" / "tfidf_database_real.pkl"
...
# --- Stage 2: 生成整体市场报告 (修正版：GPT-4o + 纯粹流式) ---
        ...
        try:
            # 3. 发起请求 (关键修改点)
            # 使用你测试成功的 gpt-4o
            # 设置 60秒超时：因为提示词很长，AI需要时间"阅读"才能吐出第一个字
            ...
            # 如果出错，直接告诉前端，不再尝试降级，因为我们知道API是好的
