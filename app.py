    ...
    try:
        ...
        for i in range(0, total_personas, batch_size):
            ...
            for j, p in enumerate(batch_personas):
                ...
                # --- 打印 Ground Truth ---
                print(f"\n[{job_id}] >>> 正在验证画像 ID_{persona_id} 的 Ground Truth (真实相似用户数据) <<<", flush=True)
                if not top_matches.empty:
                    for idx, row in top_matches.iterrows():
                        print(f"  [真实用户 {row['用户ID']}] ({row['城市']}/{row['职业']}/{row['收入区间']}):", flush=True)
                        print(f"    > 茅台飞天(2800): {row.get('决策_茅台飞天_2800元', 'N/A')}", flush=True)
                        print(f"    > 江小白(138):    {row.get('决策_江小白表达瓶_138元', 'N/A')}", flush=True)
                else:
                    print("  [提示] 未找到相似的真实用户数据。", flush=True)
                print("-" * 60, flush=True)
            ...
            # 💡 修改点：Prompt 增加“不确定”选项
            batch_decision_prompt = (
                "你是一位精通中国酒桌文化与消费心理学的市场分析师。你的任务是**预测**以下用户面对特定产品时的购买决策。\n"
                "注意：你无法看到用户的历史购买记录，你必须根据他们的画像（收入、职业、城市、性格）进行**逻辑推演**。\n\n"
                f"产品信息：'{product_description}'\n产品图片已提供。\n\n"
                f"以下是本批次 {len(batch_personas)} 位用户的画像及相似人群特征：\n\n" + "\n\n".join(persona_summaries_for_prompt) + "\n\n"
                "任务：请为每一位用户做出独立的购买决策。\n"
                "**重要决策逻辑（请严格遵守）：**\n"
                "1. **拒绝模棱两可**：请尽可能做出 '购买' 或 '不购买' 的明确判断。只有在用户特征与产品属性**极度冲突且无法调和**时，才选择 '不确定'。\n"
                "2. **面子与社交刚需**：在中国文化中，对于高价名酒（如茅台），**职业社交属性（如企业管理、商务销售）往往凌驾于个人收入和口味偏好之上**。如果用户有商务宴请需求，即使收入不高或嫌贵，也倾向于 '购买'。\n"
                "3. **价格锚点**：对于低价光瓶酒（如江小白），**年轻群体（学生、初入职场）和非商务场景**是核心驱动力。如果用户是追求高端面子的管理者，即使买得起，也可能因为档次问题选择 '不购买'。\n"
                "4. **口嫌体正直**：如果用户的‘心理价位’低于产品价格，但其‘职业’或‘城市’显示其处于高消费圈层，请预测为 '购买'（视为面子消费）。\n"
                "你的输出必须是一个严格的JSON对象，包含 `decisions` 数组。\n"
                "数组元素键：`persona_id` (整数) 和 `decision` (字符串 '购买'、'不购买' 或 '不确定')。"
            )
        ...
        # --- Stage 2: 生成整体市场报告 (修正版：GPT-4o + 纯粹流式) ---
        ...
        # 💡 修改点：Summary Prompt 增加对“不确定”人群的关注
        structured_summary_prompt = (
            ...
            f"1. 核心洞察与购买意向\n- 首先，精确计算并明确展示总体购买率、不购买率及**观望（不确定）比例**。\n- 提炼出本次模拟中最关键、最核心的市场洞察是什么。\n\n"
            f"2. 关键购买驱动力分析\n- 深入总结吸引用户做出“购买”决策的核心因素。请从产品属性、包装设计、品牌定位和价格感知等多个维度进行剖析。\n\n"
            f"3. 主要购买壁垒与犹豫点剖析\n- 剖析导致用户“不购买”的障碍，以及**导致用户“不确定”的关键犹豫点**是什么。\n\n"
            f"4. 综合结论与市场策略建议\n- 对产品的市场潜力给出一个简洁有力的综合结论。\n- 基于以上所有分析，为该产品的市场策略提供1-2条具体的、可执行的建议（特别针对如何转化‘不确定’用户）。"
            f"再次强调重要：输出中任何地方不能包含任何Markdown符号（如`*`、`#`、`-`）。"
        )
        
        q.put({"type": "summary_report_start"})
        print(f"[{job_id}] Debug: Sent summary_report_start signal. Connecting to LLM...", flush=True)
        ...
        try:
            ...
            print(f"[{job_id}] Debug: Connection established. Receiving chunks...", flush=True)
            ...
            print(f"[{job_id}] Progress: 整体报告流式传输完成。总长度: {len(summary_report_text)}", flush=True)

        except Exception as e:
            print(f"[{job_id}] ❌ 流式生成出错: {e}", flush=True)
            error_msg = f"\n[系统错误: {str(e)}]\n"
        ...
        # 💡 修改点：增加对“不确定”人群的分析（这对于营销转化很重要）
        if neutral_results:
            neutral_gender_data = dict(Counter(p['persona_details']['gender'] for p in neutral_results))
            if neutral_gender_data: generate_and_stream_chart_ux("neutral_gender", "潜在/不确定用户性别分布", "pie", neutral_gender_data)
