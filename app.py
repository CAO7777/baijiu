...
try:
    ...
    if not Path(font_path).exists(): font_path = '/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc'
    ...
    print(f"警告：字体文件 {font_path} 未找到，请确保其在项目根目录或指定正确路径。图表可能无法显示中文。")
    # 备用字体
...
# --- Vector Search (Hugging Face / FAISS Version) ---
...
def vectorize_database():
    ...
    with open(VECTOR_METADATA_PATH, 'wb') as f: pickle.dump(metadata, f)
    print("向量数据库创建成功！")

def find_similar_users_knn(persona, top_n=5):
    global embedding_model
    if not VECTOR_INDEX_PATH.exists(): return pd.DataFrame(), {}
    
    if embedding_model is None:
        embedding_model = SentenceTransformer('paraphrase-multilingual-mpnet-base-v2')

    index = faiss.read_index(str(VECTOR_INDEX_PATH))
    with open(VECTOR_METADATA_PATH, 'rb') as f: metadata = pickle.load(f)
...
def long_running_analysis(job_id, personas, product_data):
    ...
    try:
        ...
        for i, persona in enumerate(personas):
            ...
            # 从JSON中提取数据
            report_text = analysis_data.get("structured_report", {})
            ...
            result_package = {
                ...
                "data": { 
                    "persona_id": i + 1,
                    "report": report_text, # 这是一个包含多个分析部分的对象
                    "final_decision": {"decision": decision, "reason": analysis_data.get("reason", "")},
        ...
        if len(all_results) > 10:
            ...
            for item in all_results:
                ...
                persona_summary = (
                    ...
                    f"{p.get('age')}岁{p.get('gender')}，"
                    f"来自{p.get('city', '未知城市')}，"
                    f"职业为{p.get('profession', '未知')}，"
                    f"MBTI为{p.get('mbti', '未知')}，"
                    f"年收入{p.get('income', '未知')}。"
                )
                summary_context_lines.append(persona_summary)
            all_reports_text = "\n".join(summary_context_lines)
        else:
            print(f"数字人数量 ({len(all_results)}) <= 10，使用完整版上下文进行汇总。")
            all_reports_text = "\n\n".join([f"--- 用户 {item['persona_id']} ({item['decision']}) 报告 ---\n{item['report']}" for item in all_results])
        ...
        if buying_results:
            ...
            if income_counts:
                ...
                if sorted_income:
                    generate_and_stream_chart_ux("buyer_income", "购买用户收入分布", "pie", sorted_income)

        # 3. Non-Buyer Charts
        if non_buying_results:
            nonbuyer_gender_data = dict(Counter(p['persona_details']['gender'] for p in non_buying_results))
            if nonbuyer_gender_data:
                generate_and_stream_chart_ux("nonbuyer_gender", "未购买用户性别分布", "pie", nonbuyer_gender_data)
