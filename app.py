...
try:
    ...
    if not Path(font_path).exists():
        raise FileNotFoundError(f"Font file not found in project root: {font_path}")
    
    # Add the font to Matplotlib's font manager
    fm.fontManager.addfont(font_path)
    
    # Set the font properties object we will use
    ...
    print(f"Chinese font '{font_prop.get_name()}' loaded successfully from {font_path}")

except Exception as e:
    print(f"WARNING: Could not load local font file. Error: {e}")
    # Fallback to a generic font name that might be installed by build.sh
    plt.rcParams['font.family'] = ['WenQuanYi Zen Hei', 'SimHei', 'sans-serif']
    plt.rcParams['axes.unicode_minus'] = False
    font_prop = None # Set to None if local font fails
    print("WARNING: Falling back to system fonts. Chinese characters may not display correctly.")
    ...
    try:
        ...
        for i, persona in enumerate(personas):
            ...
            structured_individual_prompt = (
                ...
                f"再次强调重要：输出中任何地方不能包含任何Markdown符号（如`*`、`#`、`-`）。请使用中文。"
            )
            
            analysis_data = None
            MAX_ATTEMPTS = 3  # 最多尝试次数（初次 + 1次重试）

            for attempt in range(MAX_ATTEMPTS):
                try:
                    print(f"--- 正在为画像 {i+1} 进行第 {attempt + 1}/{MAX_ATTEMPTS} 次分析尝试... ---")
                    ...
                    try:
                        ...
                        # 如果失败，尝试智能提取
                        print(f"尝试 {attempt + 1} 失败: AI返回的不是纯净JSON，尝试智能提取...")
                        ...
                        if match:
                            ...
                            raise ValueError(f"AI返回内容中未找到有效的JSON对象。")
                    
                    print(f"--- 画像 {i+1} 在第 {attempt + 1} 次尝试时分析成功！ ---")
                    break  # 如果成功，就跳出重试循环
                    ...
                    if attempt < MAX_ATTEMPTS - 1:
                        ...
                        # 所有尝试都失败后，生成错误报告
                        print(f"--- 画像 {i+1} 的所有尝试均失败，生成错误报告。 ---")
                        analysis_data = {
                            "structured_report": { "packaging_analysis": "分析失败", "fit_analysis": "分析失败", "scenario_analysis": "分析失败" },
                            "radar_scores": {"包装": 0, "价格": 0, "香型": 0, "场景": 0},
                            "decision": "不购买",
                            "reason": f"AI在多次尝试后仍无法完成对此画像的分析，具体错误: {e}"
                        }
            ...
            result_package = {"type": "individual_report", "data": { "persona_id": i + 1, "report": cleaned_report_text, "final_decision": {"decision": decision, "reason": reason}, "radar_chart": radar_chart_b64, "persona_details": persona, "decision": decision }}
...
def generate_chart_base64(chart_type, data, title):
    ...
    if chart_type == 'pie':
        ...
        ax.set_yticklabels(["", "2", "4", "6", "8", "10"], color="grey", size=10, fontproperties=font_prop if font_prop else None)
        ax.set_ylim(0, 10)
